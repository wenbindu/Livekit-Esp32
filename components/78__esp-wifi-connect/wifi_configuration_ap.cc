...
extern const uint8_t bo_wifi_jpg_start[] asm("_binary_bo_wifi_jpg_start");
extern const uint8_t bo_wifi_jpg_end[] asm("_binary_bo_wifi_jpg_end");
...
void WifiConfigurationAp::StartWebServer()
{
    ...
    httpd_uri_t mascot_jpg = {
        ...
        .handler = [](httpd_req_t *req) -> esp_err_t {
            ...
            httpd_resp_set_hdr(req, "Connection", "close");
            httpd_resp_send(req,
                reinterpret_cast<const char*>(bo_wifi_jpg_start),
                bo_wifi_jpg_end - bo_wifi_jpg_start);
            return ESP_OK;
        },
        .user_ctx = NULL
    };
    ESP_ERROR_CHECK(httpd_register_uri_handler(server_, &mascot_jpg));
    ...
    httpd_uri_t exit_config = {
        ...
        .handler = [](httpd_req_t *req) -> esp_err_t {
            ...
            xTaskCreate([](void *ctx) {
                // Keep the delay short so the browser has time to receive the
                // response without making the restart feel sluggish.
