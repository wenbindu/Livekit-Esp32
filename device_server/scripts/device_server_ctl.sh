...
Usage:
  ...
  bash device_server/scripts/device_server_ctl.sh restart
...
start_server() {
    ...
    collect_pids
    if [[ "${#PIDS[@]}" -gt 0 ]]; then
        echo "device server already running: pid=${PIDS[0]}"
        print_endpoints
        return 0
    fi
    ...
    for _i in $(seq 1 20); do
        if ! pid_is_running "${server_pid}"; then
            echo "failed to start device server" >&2
        ...
        if command -v lsof >/dev/null 2>&1; then
            if lsof -nP -a -p "${server_pid}" -iTCP:"${PORT}" -sTCP:LISTEN >/dev/null 2>&1; then
                echo "device server started: pid=${server_pid}"
                print_endpoints
                return 0
            fi
        else
    ...
    echo "device server did not bind to port ${PORT}" >&2
    [[ -f "${LOG_FILE}" ]] && tail -n 40 "${LOG_FILE}" >&2
    rm -f "${PID_FILE}"
    return 1
}
...
stop_server() {
    ...
    if [[ "${#PIDS[@]}" -eq 0 ]]; then
        rm -f "${PID_FILE}"
        echo "device server is not running"
        return 0
    fi
    ...
    rm -f "${PID_FILE}"
    echo "device server force stopped"
    return 0
}
...
status_server() {
    ...
    echo "device server is running: pid=${PIDS[0]}"
    print_endpoints
    return 0
}
